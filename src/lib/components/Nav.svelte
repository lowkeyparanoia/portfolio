<script>
  ...
  function navClick(href) {
    ...
    if (bar && overlay) {
      ...
      bar.style.animation   = 'scanDown 280ms linear forwards';
      setTimeout(() => {
...
<div id="scan-overlay" style="position:fixed;inset:0;background:rgba(0,255,65,0.04);pointer-events:none;opacity:0;transition:opacity 0.12s;z-index:9998;"></div>
<div id="scan-bar" style="position:fixed;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#00ff41,transparent);pointer-events:none;z-index:9999;"></div>
